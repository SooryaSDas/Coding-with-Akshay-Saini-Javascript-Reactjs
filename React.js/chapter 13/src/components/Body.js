  ...
  // if(filteredrestaurants?.length === 0) return <h2>no restaurant are found</h2>;
    ...
    return allrestaurant.length === 0 ? <Shimmer/> :(
       ...
       <div className="search-container p-5 bg-pink-50 my-2">  
        ...
        <button data-testid = "search-btn" className="p-2 m-2 bg-green-500 hover:bg-green-950 text-white rounded-md" 
        onClick={()=>{
                ...
                filteredrestaurants.map((restaurant)=>{
                   return(
                    <Link to={"/restaurant/" + restaurant.data.id} key={restaurant.data.id}>
                      <RestaurantCard {...restaurant.data} key={restaurant.key}/>
                    </Link>
                   )
                })
            }
